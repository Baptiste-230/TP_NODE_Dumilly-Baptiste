...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .setDescription('Ajoute un élément à la watchlist.')
        ...
        .addIntegerOption(option =>
            option.setName('film-id')
                .setDescription('Nom du film à ajouter à la watchlist.')
                .setRequired(true))
    ...
    /**
     * Exécute la commande slash pour ajouter un élément à la watchlist.
    ...
    async execute(interaction) {
        try {
            ...
            let response;
            try {
                response = await axios.patch('http://localhost:3000/watchlists/addItem', {
                ...
                if (error.response && error.response.data && error.response.data.error) {
                    ...
                    await interaction.reply('Une erreur s\'est produite lors de l\'ajout de l\'élément à la watchlist.');
                }
                return;
            }

            // Si la requête est réussie, envoyer une réponse dans le chat Discord
            await interaction.reply('Élément ajouté à la watchlist avec succès!');
        } catch (error) {
            await interaction.reply('Une erreur s\'est produite lors de l\'ajout de l\'élément à la watchlist.');
        }
    },
